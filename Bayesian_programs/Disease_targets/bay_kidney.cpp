...
// g++ bay_kidney.cpp -o bay_kidney
// ./bay_kidney nr_report.txt ICD10_kidney MSDRG_917 ICD10_A41 
//
//
...
while(!in.eof())  // read in the matrix
	...
	for (int s=1; s<(h);s++)  // new_word=stroke
		{
   		if (strcmp(word2[s],"ICD10_N17")==0) { strcpy (word2[s],new_word); flag=flag+1;}
    	if (strcmp(word2[s],"ICD10_N18")==0) { strcpy (word2[s],new_word); flag=flag+1;}
    	if (strcmp(word2[s],"ICD10_N19")==0) { strcpy (word2[s],new_word); flag=flag+1;}
    	if (strcmp(word2[s],"ICD9_585")==0)  { strcpy (word2[s],new_word); flag=flag+1;}
    	if (strcmp(word2[s],"ICD9_584")==0)  { strcpy (word2[s],new_word); flag=flag+1;}
