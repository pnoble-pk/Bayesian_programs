...
while(!in.eof())  // read in the matrix
	...
	for (int s=1; s<(h);s++)  // new_word=stroke
		{
   		if (strcmp(word2[s],"ICD10_F17")==0)  { strcpy (word2[s],"ICD10_Smoke");flag=flag+1;}
   		if (strcmp(word2[s],"ICD9_305")==0)  { strcpy (word2[s],"ICD10_Smoke");flag=flag+1;}
