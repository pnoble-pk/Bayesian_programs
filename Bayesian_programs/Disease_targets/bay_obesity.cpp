...
// g++ bay_obesity.cpp -o bay_obesity
// ./bay_obesity nr_test.txt ICD10_Obesity ICD10_F11 ICD10_F32
//
...
while(!in.eof())  // read in the matrix
	...
	for (int s=1; s<(h);s++)  // new_word=stroke
		{
 		if (strcmp(word2[s],"ICD10_E66")==0) { strcpy (word2[s],new_word); flag=flag+1;}
    	if (strcmp(word2[s],"ICD10_Z68")==0) { strcpy (word2[s],new_word); flag=flag+1;}
    	if (strcmp(word2[s],"ICD9_278")==0) { strcpy (word2[s],new_word); flag=flag+1;}
